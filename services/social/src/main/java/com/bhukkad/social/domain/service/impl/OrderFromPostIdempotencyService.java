...
/**
 * Idempotency service for order-from-post endpoint.
 * 
 * <p>Replay guard for the order-from-post surface when an {@code Idempotency-Key}
 ...
 * ({@link DuplicateRequestException}) — never a second order creation.
 * 
 ...
 * and a saga rollback cannot undo the dedupe bookkeeping.
 */
@Slf4j
@Service
@RequiredArgsConstructor
public class OrderFromPostIdempotencyService {
    ...
    public record Claim(boolean fresh, int httpStatus, com.bhukkad.order.api.dto.response.OrderResponse replay) {
        public static Claim ofFresh() {
            return new Claim(true, 0, null);
        }

        public static Claim ofReplay(int status, com.bhukkad.order.api.dto.response.OrderResponse body) {
    ...
    public Claim claim(Long customerId, Long postId, String idempotencyKey, OrderFromPostRequest request) {
        String requestHash = hashOf(customerId, postId, request);
    ...
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void complete(Long customerId, Long postId, String idempotencyKey, OrderFromPostRequest request,
                         int httpStatus, com.bhukkad.order.api.dto.response.OrderResponse body) {
        ...
        try {
            ...
            record.setResponsePayload(objectMapper.writeValueAsString(
                    new StoredEnvelope(hashOf(customerId, postId, request), httpStatus, body)));
            ...
            throw new IllegalStateException(
                    "Failed to serialize idempotent order-from-post response", serializationFailure);
        }
    }
     ...
     */
    record StoredEnvelope(String requestHash, Integer httpStatus, com.bhukkad.order.api.dto.response.OrderResponse body) {
    }
     ...
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public void markFailed(Long customerId, Long postId, String idempotencyKey) {
        IdempotencyRecord record = requireOwnedClaim(customerId, idempotencyKey);
     ...
     * subject, the post being ordered from, and the explicit item list).
     */
    String hashOf(Long customerId, Long postId, OrderFromPostRequest request) {
        Map<String, Object> canonical = new LinkedHashMap<>();
        canonical.put("customerId", customerId);
        canonical.put("postId", postId);
