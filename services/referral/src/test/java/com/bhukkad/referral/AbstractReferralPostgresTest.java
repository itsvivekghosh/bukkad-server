...
/**
 * Base class for the referral service's PostgreSQL integration tests
 * (mirrors {@code AbstractIdentityPostgresTest} — referral has no prior
 * harness). Boots {@code postgres:16-alpine}, pins the dialect, and restricts
 * Flyway to the referral baseline + service migrations
 * ({@code classpath:db/migration-pg}).
 */
@Testcontainers(disabledWithoutDocker = true)
public abstract class AbstractReferralPostgresTest {
    ...
    static void postgresProperties(DynamicPropertyRegistry registry) {
        ...
        // Deterministic abuse ceilings inside the suite (override the defaults).
        registry.add("app.referral.abuse.apply-per-customer-per-day", () -> "1000");
    }
}
