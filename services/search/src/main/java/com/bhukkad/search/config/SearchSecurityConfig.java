...
/**
 * Search service security: unified search and autocomplete are public reads;
 * the internal index upsert stays authenticated (service-to-service via basic
 * auth until JWT propagation lands with the identity extraction).
 */
@Configuration
@EnableWebSecurity
...
@Bean
     public SecurityFilterChain searchSecurityFilterChain(HttpSecurity http,
                                                          ...
                                                          ObjectProvider<ServiceJwtAuthFilter> serviceJwtAuthFilter) throws Exception {
         http
